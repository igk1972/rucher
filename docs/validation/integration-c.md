...
`/var/lib/rucher/agent-status.json`); `rucher` built for linux/arm64 and present on the nodes;
...
1. `rucher hosts status` (from the directory with `./hosts`) → a HOST/ADDRESS/REACHABLE/REVISION/
...
2. `rucher hosts status --live lima-essaim-01` → plus a `rucher status` block with live ActiveState/SubState.
3. `rucher net join lima-essaim-01 --address 100.64.0.1` → in
