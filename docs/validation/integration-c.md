...
1. `rucher ops nodes status` (from the directory with `./nodes`) → a NODE/ADDRESS/REACHABLE/REVISION/
...
2. `rucher ops nodes status --live lima-essaim-01` → plus a `rucher node cadre status` block with live ActiveState/SubState.
3. `rucher ops nodes join lima-essaim-01 --address 100.64.0.1` → in
   `./nodes/lima-essaim-01/configuration.yml` a `network: {address: 100.64.0.1}` appears,
   other keys/comments left in place. A repeated `ops nodes join` with a different address updates the value.
