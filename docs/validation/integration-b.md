...
Validates the GitOps flow: node key → seal → git store → `node agent run` → apply, and removal via
...
for linux and at `/usr/local/bin/rucher`. The store is a git repository reachable by the node: a local
...
7. Removal: `placement.yml` → `web: lima-essaim-02`, commit, `sudo rucher node agent run` → `removed=[web]`,
   units/container/state removed, user `rucher-web` retained (rm without purge). `rucher node cadre rm web --purge`
   removes the user.
8. `sudo rucher node agent install` → `rucher-agent.timer` active
