...
| `secrets.example.sops.yaml` | PLAINTEXT template for the authkey; encrypt it into `secrets.sops.yaml`, never commit the real key. The `.sops.yaml` suffix marks it a service file, so `apply` never copies this template onto the node. |
...
sudo rucher node cadre recipient overlay-demo         # -> age1...
...
sudo rucher node cadre apply --dir . overlay-demo
```
